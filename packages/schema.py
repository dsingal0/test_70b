...
class ModelInput:
    ...
    def to_tensors(self):
        inputs = [
            self._prepare_grpc_tensor(
                "text_input", np.array([[self._prompt]], dtype=object)
            ),
            self._prepare_grpc_tensor(
                "max_tokens", np.array([[self._max_tokens]], dtype=np.int32)
            ),
        ]
        ...
        if self._decoder_input is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    "decoder_text_input",
                    np.array([[self._decoder_input]], dtype=object),
                )
            )
        ...
        if self._bad_words_list:
            inputs.append(
                self._prepare_grpc_tensor(
                    "bad_words", np.array([self._bad_words_list], dtype=object)
                )
            )
        ...
        if self._stop_words_list:
            inputs.append(
                self._prepare_grpc_tensor(
                    "stop_words", np.array([self._stop_words_list], dtype=object)
                )
            )
        ...
        if self._pad_id is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    "pad_id", np.array([[self._pad_id]], dtype=np.int32)
                )
            )
        ...
        inputs.extend(
            [
                self._prepare_grpc_tensor(
                    "top_k", np.array([[self._top_k]], dtype=np.int32)
                ),
                self._prepare_grpc_tensor(
                    "top_p", np.array([[self._top_p]], dtype=np.float32)
                ),
                self._prepare_grpc_tensor(
                    "temperature", np.array([[self._temperature]], dtype=np.float32)
                ),
                self._prepare_grpc_tensor(
                    "length_penalty",
                    np.array([[self._length_penalty]], dtype=np.float32),
                ),
                self._prepare_grpc_tensor(
                    "repetition_penalty",
                    np.array([[self._repetition_penalty]], dtype=np.float32),
                ),
                self._prepare_grpc_tensor(
                    "min_length", np.array([[self._min_length]], dtype=np.int32)
                ),
                self._prepare_grpc_tensor(
                    "presence_penalty",
                    np.array([[self._presence_penalty]], dtype=np.float32),
                ),
                self._prepare_grpc_tensor(
                    "frequency_penalty",
                    np.array([[self._frequency_penalty]], dtype=np.float32),
                ),
                self._prepare_grpc_tensor(
                    "return_log_probs", np.array([[self._return_log_probs]], dtype=bool)
                ),
                self._prepare_grpc_tensor(
                    "return_context_logits",
                    np.array([[self._return_context_logits]], dtype=bool),
                ),
                self._prepare_grpc_tensor(
                    "return_generation_logits",
                    np.array([[self._return_generation_logits]], dtype=bool),
                ),
                self._prepare_grpc_tensor(
                    "beam_width", np.array([[self._beam_width]], dtype=np.int32)
                ),
                self._prepare_grpc_tensor(
                    "stream", np.array([[self._stream]], dtype=bool)
                ),
            ]
        )
        ...
        if self._random_seed is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    "random_seed", np.array([[self._random_seed]], dtype=np.uint64)
                )
            )
        ...
        if self._prompt_embedding_table is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    "prompt_embedding_table", self._prompt_embedding_table
                )
            )
        ...
        if self._prompt_vocab_size is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    "prompt_vocab_size",
                    np.array([[self._prompt_vocab_size]], dtype=np.int32),
                )
            )
        ...
        if self._embedding_bias_words is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    ...
                    np.array([self._embedding_bias_words], dtype=object),
                )
            )
        ...
        if self._embedding_bias_weights is not None:
            inputs.append(
                self._prepare_grpc_tensor(
                    ...
                    np.array([self._embedding_bias_weights], dtype=np.float32),
                )
            )
